...
void CustomPartitionFrame::slotCreatePartition(Device::Ptr dev, Partition::Ptr part)
{
    ...
    m_createPartFrame = new CreatePartitionFrame(m_delegate,&devlist);
    connect(m_createPartFrame, &CreatePartitionFrame::finished, [=]{repaintDevice();m_createPartFrame->close();});
    m_createPartFrame->setNewPartition(dev, part, bl);
...
void CustomPartitionFrame::slotModifyPartition(Device::Ptr dev, Partition::Ptr part)
{
    ...
    if(!m_delegate->isPartitionTableMatchDe(dev_path)) {
        ...
        m_modifyPartFrame = new ModifyPartitionFrame(m_delegate);
        m_modifyPartFrame->devlst=&devlist;//把硬盘列表传过去
        connect(m_modifyPartFrame, &ModifyPartitionFrame::finished, [=]{repaintDevice();m_modifyPartFrame->close();});
        m_modifyPartFrame->setModifyPartition(part);
...
{
    m_delegate->setBootLoaderPath(bootstr);
    m_bootstr = bootstr;
}
