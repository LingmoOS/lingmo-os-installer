...
do_copy_config() {
  ...
  if grep -q automatic /proc/cmdline; then
    [[ -f /cdrom/lingmo-installer.cfg ]] && cp /cdrom/lingmo-installer.cfg /tmp
  # 990/9A0 使用此设置，ple-mode 审核模式，test-mode 全自动安装模式
  elif grep -qE 'ple-mode|test-mode' /proc/cmdline; then
    [[ -f /cdrom/kyple-installer.cfg ]] && cp /cdrom/kyple-installer.cfg /tmp/lingmo-installer.cfg
  else
    cp /usr/share/lingmo-os-installer/lingmo-installer.cfg /tmp
  fi
}
...
do_run_target() {
  ...
  cp utils /tmp/lingmo-installer.cfg /target/tmp
